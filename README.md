...
| :------------------ | :----------: |
| Plugin name | raft_heatmap |
| Plugin name slugified | raft_heatmap |
| Plugin name class (used in code) | RaftHeatmap |
| Plugin category | None |
| Plugin description short | QGIS plugin that streamlines heatmap generation for SDRPF RAFT data |
| Plugin description long | A custom plugin for quick and easy map generation of SDRPF RAFT map data |
| Plugin tags | heatmap,workflow automation |
| Plugin icon | default_icon.png |
| Plugin with processing provider | False |
| Author name | Alexandra Walton Weston |
| Author organization | None |
| Author email | alexwweston@gmail.com |
| Minimum QGIS version | 3.44 |
| Maximum QGIS version | 3.99 |
| Support Qt6 | True |
| Git repository URL | https://github.com/alxwstn/RAFTHeatmapMkr |
...
> The final URL should be something like this: <https://{user_org}.github.io/{project_slug}>. You can find it in Pages settings of your repository: <https://github.com/alxwstn/RAFTHeatmapMkr/pages>.

1. Have a look to the [plugin's metadata.txt file](raft_heatmap/metadata.txt): review it, complete it or fix it if needed (URLs, etc.)., especially the `homepage` URL which should be to your GitLab or GitHub Pages.
1. Update the base URL of custom repository in [installation doc page](./docs/usage/installation.md).
1. Change the plugin's icon stored in `raft_heatmap/resources/images`
...
1. Switch the source of GitHub Pages to `GitHub Actions` in your repository settings <https://github.com/alxwstn/RAFTHeatmapMkr/pages>
1. Add the remote repository to your local repository:

    ```sh
    git remote add origin https://github.com/alxwstn/RAFTHeatmapMkr
    ```

1. Commit changes:

    ```sh
    git commit -m "init(plugin): adding first files of raft_heatmap" -m "generated with QGIS Plugin Templater (https://oslandia.gitlab.io/qgis/template-qgis-plugin)"
    ```
