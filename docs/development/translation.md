...
1. Update `.ts` files:
    ...
    ```bash
    pylupdate5 -noobsolete -verbose raft_heatmap/resources/i18n/plugin_translation.pro
    ```

1. Translate your text using QLinguist or directly into `.ts` files. Launching it through command-line is possible:

    ```bash
    linguist raft_heatmap/resources/i18n/*.ts
    ```

1. Compile it:

    ```bash
    lrelease raft_heatmap/resources/i18n/*.ts
    ```
